    ...
    <Wrapper>
      ...
      <ButtonBox>
        {btnValues.flat().map((btn, id) => {
          return <Button key={id} className={btn === "=" ? "equals" : ""} value={btn} onClick={btn === "C" ? resetClickHandler : btn === "." ? commaClickHandler : btn === "+-" ? invertClickHandler : btn === "%" ? percentClickHandler : btn === "=" ? equalsClickHandler : btn === "/" || btn === "X" || btn === "-" || btn === "+" ? signClickHandler : numClickHandler} />;
        })}
      </ButtonBox>
    </Wrapper>
  );
